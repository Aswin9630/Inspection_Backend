...
// Helper: prefer secure_url/path/url; normalize Cloudinary PDF URL to raw delivery if necessary
...
//       }
//       if (url.includes("/upload/") && /\.pdf($|\?)/i.test(url)) {
...
// function extractCloudinaryUrl(fileObj) {
//   if (!fileObj) return null;
//   const url = fileObj.secure_url || fileObj.path || fileObj.url || null;
//   return normalizeCloudinaryPdfUrl(url);
// }
...
//     const tradeFile = req.files?.tradeLicense?.[0];
//     const importFile = req.files?.importExportCertificate?.[0];

//     const tradeUrl = extractCloudinaryUrl(tradeFile);
//     const importUrl = extractCloudinaryUrl(importFile);

//     const updates = {};
//     if (tradeUrl) updates["documents.tradeLicense"] = tradeUrl;
//     if (importUrl) updates["documents.importExportCertificate"] = importUrl;

//     if (Object.keys(updates).length === 0) {
//       return next(errorHandler(400, "No documents uploaded"));
//     }

//     const customer = await Customer.findById(req.user._id).select("documents publishRequirements").lean();
//     if (!customer) return next(errorHandler(404, "Customer not found"));

//     const currentTrade = updates["documents.tradeLicense"] || customer.documents?.tradeLicense || null;
//     const currentImport =
//       updates["documents.importExportCertificate"] || customer.documents?.importExportCertificate || null;

//     if (currentTrade && currentImport) {
//       updates.publishRequirements = true;
//     }
...
//     });
//   } catch (error) {
//     return next(errorHandler(500, "Failed to update documents: " + (error?.message || error)));
//   }
// };
