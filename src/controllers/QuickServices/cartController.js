...
// const { paymentUser, paymentTeam, enquiryUser,enquiryTeam } = require("../../utils/EmailServices/QuickServiceEmail/index"); 
// const EMAIL_FROM = process.env.EMAIL_FROM || `Qualty.ai <${process.env.EMAIL_USER}>`;
// const TEAM_EMAIL = process.env.TEAM_EMAIL || "team@qualty.ai";
// const SUPPORT_EMAIL = process.env.SUPPORT_EMAIL || "support@qualty.ai";
// const ADMIN_BASE_URL = process.env.ADMIN_BASE_URL || "https://app.qualty.ai/admin";
// const TAX_RATES = { IN: 0.18, DEFAULT: 0.0 };
...
//       razorpay_payment_id,
//       razorpay_signature,
//       amount: fetchedPayment?.amount || 0, // smallest unit
//       currency: fetchedPayment?.currency || (order_meta?.currency || "INR"),
//       status: fetchedPayment?.status || "captured",
//       metadata: {
//         fetchedPayment,
...
//     // Build order document from server cart snapshot (authoritative)
//     const cartItems = await CartItem.find({ userId: user._id });
...
//       // send payment emails (even if no items)
//       try {
//         const amountMajor = ((paymentDoc.amount || 0) / 100).toFixed(2);
//         const amountLabel = paymentDoc.currency === "USD" ? `$${amountMajor}` : `₹${amountMajor}`;
//         const userTpl = paymentUser({
//           customerName: user.name || user.email || "Customer",
//           amount: amountLabel,
//           orderId: orderDoc._id,
//           paymentId: paymentDoc.razorpay_payment_id || paymentDoc._id,
//           date: new Date().toLocaleString(),
//           serviceSummary: "Quick Service",
//           supportEmail: SUPPORT_EMAIL,
//         });
//         const teamTpl = paymentTeam({
//           customerName: user.name || user.email || "Customer",
...
//           date: new Date().toLocaleString(),
//           serviceSummary: "Quick Service",
//           adminOrderUrl: `${ADMIN_BASE_URL}/orders/${orderDoc._id}`,
//         });

//         await Promise.all([
//           safeSendMail({ to: user.email, subject: userTpl.subject, text: userTpl.text, html: userTpl.html }),
//           safeSendMail({ to: TEAM_EMAIL, subject: teamTpl.subject, text: teamTpl.text, html: teamTpl.html }),
//         ]);
//       } catch (mailErr) {
//         console.warn("Failed to send payment emails:", mailErr);
//       }
...
//     // Send payment emails (user + team)
//     try {
//       const amountMajor = (totalMajor).toFixed(2);
//       const amountLabel = orderDoc.currency === "USD" ? `$${amountMajor}` : `₹${amountMajor}`;
//       const serviceSummary = itemsSnapshot.map((it) => `${it.serviceType} @ ${it.location}`).join("; ") || "Quick Service";
//       const userTpl = paymentUser({
//         customerName: user.name || user.email || "Customer",
//         amount: amountLabel,
//         orderId: orderDoc._id,
...
//     // 1) Try exact match country+region (strict)
//     let loc = await LocationPrice.findOne({
//       country: new RegExp(`^${escapeRegex(countryNorm)}$`, "i"),
//       region: new RegExp(`^${escapeRegex(regionNorm)}$`, "i"),
//     }).lean();

//     // 2) If not found, try fuzzy region with same country (still requires country match)
//     if (!loc) {
//       loc = await LocationPrice.findOne({
//         country: new RegExp(`^${escapeRegex(countryNorm)}$`, "i"),
//         region: new RegExp(escapeRegex(regionNorm), "i"),
...
//     // DO NOT fall back to a country-only DB record here. Treat 'no loc' as region missing.

//     // Check if provided country string exists in DB (exact country match)
//     const countryExists = await LocationPrice.exists({
//       country: new RegExp(`^${escapeRegex(countryNorm)}$`, "i"),
//     });

//     // Heuristic: treat variations containing 'india' as India (covers typos like 'lindia')
//     const looksLikeIndia = String(countryNorm).toLowerCase().includes("india") || String(countryNorm).toLowerCase() === "in";

//     // Decide final price/currency/isEnquiry
//     let price = 0;
//     let currency = "INR";
//     let isEnquiry = true;
...
//         price = 2500;
//         currency = "INR";
//         isEnquiry = false; // payable fallback
...
//     // Send enquiry / fallback emails when region not present
//     const customerName = req.user?.name || req.user?.email || "Customer";
//     const customerEmail = req.user?.email || "";
//     const cartItemId = cartItem._id;
//     const region = regionNorm;
//     const countr = countryNorm;
//     const serviceSummary = `${serviceType} @ ${region}`;

//     if (!loc && fallbackApplied && !isEnquiry) {
//       // Fallback applied for Indian location (₹2500)
//       try {
//         const fallbackAmount = "₹2500";
//         const userTpl = enquiryUser({
//           customerName,
//           location: region,
//           country: countr,
//           cartItemId,
...
//           date: new Date().toLocaleString(),
//           serviceSummary,
//           adminEnquiryUrl: `${ADMIN_BASE_URL}/enquiries/${cartItemId}`,
//         });

//         await Promise.all([
//           safeSendMail({ to: customerEmail, subject: userTpl.subject, text: userTpl.text, html: userTpl.html }),
//           safeSendMail({ to: TEAM_EMAIL, subject: teamTpl.subject, text: teamTpl.text, html: teamTpl.html }),
//         ]);
//       } catch (mailErr) {
//         console.warn("addToCart fallback: failed to send emails", mailErr);
//       }
...
//         ]);
//       } catch (mailErr) {
//         console.warn("addToCart enquiry: failed to send emails", mailErr);
//       }
