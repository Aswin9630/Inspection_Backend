    ...
    (req, res, next) => {
      ...
      if (role === "customer") {
        validations = [
          ...
          check("gstNumber").custom((val, { req }) => {
            ...
            const isIndian = req.body.countryCode === "+91";
            if (shouldPublish && isIndian) {
              if (!val) throw new Error("GST number is required to publish requirements");
              if (!/^\d{2}[A-Z]{5}\d{4}[A-Z]{1}[A-Z\d]{1}Z[A-Z\d]{1}$/.test(val))
                throw new Error("Invalid GST number format");
            }
            return true;
      ...
      if (role === "inspector") {
        validations = [
          ...
          mobileRule(),
          check("address", "Address too long").optional().isLength({ max: 100 }),
          check("inspectorType", "Inspector type must be 'indian' or 'international'").isIn([
            "indian",
            "international",
          ]),
          check("acceptsRequests").optional().isBoolean(),
          check("identityDocuments.aadhaarCard").custom((value, { req }) => {
            const requiresDocs =
              req.body.acceptsRequests === "true" || req.body.acceptsRequests === true;
            const fileExists = req.files?.aadhaarCard?.length > 0;
            if (requiresDocs && !fileExists)
              throw new Error("Aadhaar card is required to accept requests");
            return true;
          ...
          check("commodities.*.commodity", "Commodity is required").notEmpty(),
          check("commodities.*.experienceYears", "Experience must be a number").isInt({
            min: 0,
            max: 50,
          }),
        ];
      ...
      if (role === "inspection_company") {
        validations = [
          ...
          check("phoneNumber", "Invalid company phone number").matches(/^\d{6,15}$/),
          mobileRule("mobileNumber"),
          check("firstName", "Contact first name is required")
            .isString()
            .isLength({ min: 1 }),
          check("lastName", "Contact last name is required")
            .isString()
            .isLength({ min: 1 }),
          countryCodeRule(),
          check("websiteUrl")
